...
allData <- cbind(sequences, encoded_seq)

write.csv(allData,"data/encoded_seq_with_seq.csv", row.names = TRUE)
write.csv(encoded_seq,"data/encoded_seq.csv", row.names = TRUE)
#pepts <- my_data_clean_aug %>% 
#  pull(peptide) %>% 
#  encode_peptide("blosum62")
