...
# Add aminoacid type
seq_properties  <- seq_properties  %>% 
  mutate(aminoacid_class = case_when(mutation ==  "R" | mutation =="H" | mutation == "K" ~ "basic", 
...
seq_properties %>% group_by(mutation) %>% 
  ggplot() + 
...
seq_properties %>% group_by(mutation_position) %>% 
  ggplot() + 
...
seq_properties %>% ggplot() + 
  geom_tile(aes(y=mutation, x=mutation_position, fill = pI)) +
  scale_fill_distiller(palette = "RdPu") +
  theme_ipsum()
#mw  
seq_properties %>% ggplot() + 
  geom_tile(aes(y=mutation, x=mutation_position, fill = MW)) +
  scale_fill_distiller(palette = "RdPu") +
  theme_ipsum()

#z5
seq_properties %>% ggplot() + 
  geom_tile(aes(y=mutation, x=mutation_position, fill = Z5)) +
  scale_fill_distiller(palette = "RdPu") +
  theme_ipsum()

#z5 grouped by type of aa
seq_properties %>% group_by(aminoacid_class) %>%  
  ggplot() + 
  geom_tile(aes(y=aminoacid_class, x=mutation_position, fill = Z5)) +
  scale_fill_distiller(palette = "RdPu") +
  theme_ipsum()

#Score
seq_properties %>% ggplot() + 
  geom_tile(aes(y=mutation, x=mutation_position, fill = score)) +
  scale_fill_distiller(palette = "RdPu") +
  theme_ipsum()
...
seq_properties %>% group_by(aminoacid_class) %>%  
  ggplot() + 
  geom_tile(aes(y=aminoacid_class, x=mutation_position, fill = score)) +
  scale_fill_distiller(palette = "RdPu") +
  theme_ipsum()
...
summary <- seq_properties %>% group_by(mutation_position) %>% 
...
seq_properties %>% group_by(mutation) %>% 
  ...
  scale_x_continuous(breaks = c(0, 0)) + 
